...
public class TimeSlotConfigForm extends HorizontalLayout {
    ...
    public TimeSlotConfigForm() {
        setSizeFull();
        binder.setBean(new TimeSlotConfig().setPeriodStart(LocalDate.now()));
        ...
        binder.forField(periodStartPicker).asRequired().bind(TimeSlotConfig::getPeriodStart,
                TimeSlotConfig::setPeriodStart);
        ...
        addTimeSlotSeriesButton.addClickListener(event -> {
            if (isValidSlotConstellation()) {
                ...
                TimeSlotConfig config = new TimeSlotConfig()
                        .setBundesland(Bundesland.byString(bundesland))
                        .setDayOfWeek(selectedDayOfWeek)
                        .setPeriodStart(periodStartPicker.getValue())
    ...
    public TimeSlotConfig getCurrentConfig() {
        if (binder.getBean() == null) {
            binder.setBean(new TimeSlotConfig().setBundesland(Bundesland.byString(bundesland)));
        }
