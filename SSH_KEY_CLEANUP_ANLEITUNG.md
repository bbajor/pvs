...
```bash
# Ersetze SSH-Key in den Dateien
sed -i 's/-----BEGIN OPENSSH PRIVATE KEY-----.*-----END OPENSSH PRIVATE KEY-----/[PRIVATE_SSH_KEY_ENTFERNT]/s' GITHUB_SECRETS_EINFACH.md
sed -i 's/-----BEGIN OPENSSH PRIVATE KEY-----.*-----END OPENSSH PRIVATE KEY-----/[PRIVATE_SSH_KEY_ENTFERNT]/s' GITHUB_SECRETS.md
```
